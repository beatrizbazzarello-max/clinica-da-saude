...
def inserir_usuario():
    ...
    if altura and nome and imc:
    #    try:
            conn  =  conectar()
            c = conn.cursor()
            c.execute('INSERT INTO usuarios (id, nome, peso, idade, altura, imc) VALUES (?,?,?,?,?,?)', (id, nome, peso, idade, altura, imc))
            conn.commit()
            conn.close()
...
def editar():
    ...
    if selecao:
        ...
        novo_imc = entry_imc.get()
       
        if  novo_nome and novo_peso:
            try:
                    conn  =  conectar()
                    c = conn.cursor()
                    c.execute('UPDATE usuarios SET nome = ?,  peso= ?, idade=?, altura =?, imc=? WHERE id = ?', (novo_nome, novo_peso, novo_idade, novo_altura, novo_imc, user_id))
                    conn.commit()
                    conn.close()
                    messagebox.showinfo('Dados','DADOS INSERIDOS COM SUCESSO!')
                    mostra_usuario()
...
entry_id.grid(row=0, column=1, padx=(0,20), pady=5, sticky='w')
...
# textos para direcionar
# IDADE
ttk.Label(input_frame, text='IDADE').grid(row=3, column=0, padx=(0,10), pady=5, sticky='e')

entry_idade = ttk.Entry(input_frame, width=30)
entry_idade.grid(row=3, column=1, padx=(0,20), pady=5, sticky='w')
...
ttk.Label(input_frame, text='IMC').grid(row=5, column=0, padx=(0,10), pady=5, sticky='e')

entry_imc = ttk.Entry(input_frame, width=30)
entry_imc.grid(row=5, column=1, padx=(0,20), pady=5, sticky='w')
...
# criação da TreeView
columns = ('ID', 'NOME', 'PESO', 'IDADE', 'ALTURA', 'IMC')
